...
dat = read.csv('../1.rawData & preprocess/testData_83.csv')

###############################################################################

model = readRDS('../2.model training/model/F_model.rds')
...
model = readRDS('../2.model training/model/BI_model.rds')
pred_logit=predict.glm(model,newdata = dat)
pred_prob=exp(pred_logit)/(1+exp(pred_logit))
...
###############################################################################
model = readRDS('../2.model training/model/NAS_model.rds')
pred_logit=predict.glm(model,newdata = dat)
pred_prob=exp(pred_logit)/(1+exp(pred_logit))
